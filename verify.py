...
robustness = json.loads((Path("data") / "exact-witness-network-robustness.json").read_text(encoding="utf-8"))
print(f"experiment56_status={robustness['status']} components={robustness['summary']['component_count']} pure_substitution={robustness['summary']['pure_substitution_count']} tau_less_kappa={robustness['summary']['tau_less_kappa_count']} residual_backbone_single_failures={len(robustness['single_failure_taxonomy']['residual_backbone_worlds'])}")
profiles = json.loads((Path("data") / "exact-degradation-profiles.json").read_text(encoding="utf-8"))
